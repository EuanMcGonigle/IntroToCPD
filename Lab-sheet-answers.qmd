...
- implement the CUSUM and MOSUM statistics from scratch, and estimate the noise level robustly in the presence of change points;
- apply the `changepoint`, `mosum` and `EnvCpt` packages, and relate their output to your own implementations;
- choose and interpret thresholds and bandwidths, and reason about false alarms (Type I error);
- use information criteria (AIC and BIC) to choose between competing change point models, and recognise when the modelling assumptions (such as independent noise) may fail.
...
(b) To set a threshold for declaring changes, we need to know the noise level $\sigma$. In reality this is unknown, so we must estimate it from the data -- but the change points themselves (and any trend) will inflate a naive estimate of the spread. Consider the following candidate estimators of $\sigma$:
...
**Check your code before trusting it.** Confirm your function behaves sensibly on data where you already know the answer, with two controlled tests.

*A change is present.* Simulate a series with one known change in mean, `set.seed(1); x <- c(rnorm(100), rnorm(100, mean = 2))`, which changes at $t = 100$. Your (scaled) statistic should peak clearly near $t = 100$ and comfortably exceed a sensible threshold.

*No change (pure noise).* Now simulate a series with **no** change at all, `set.seed(1); x <- rnorm(300)`. There is nothing to find, so your statistic should mostly stay *below* the threshold. Try a few different seeds: how often does it cross the threshold anyway? That rate is the false-alarm (Type I error) probability, and controlling it is exactly what the threshold is for.

**Solution:** Using the scaled CUSUM statistic from part (c):
...
cusum <- CUSUM.calc2(x)
which.max(cusum)                        # should be near 100
max(cusum) > sqrt(2 * log(length(x)))   # should be TRUE

# (2) pure noise, no change: repeat many times and count false alarms
set.seed(1)
false.alarms <- replicate(1000, {
  z <- rnorm(300)
  max(CUSUM.calc2(z)) > sqrt(2 * log(length(z)))
})
mean(false.alarms)                       # empirical false-alarm rate
```

For the first series the peak sits exactly at $t = 100$ and clears the threshold, confirming the statistic works. For pure noise the statistic only occasionally exceeds the threshold: here the estimated false-alarm rate is around 5%, so $D = \sqrt{2\log n}$ behaves like a test with a small significance level. Raising $D$ lowers the false-alarm rate at the cost of power to detect real changes, and lowering it does the reverse.

(d) What is the computational complexity of the code you wrote in part (a)? How fast could it be? Verify your answer empirically: use `system.time()` to time your function on series of increasing length (say $n = 10^3, 10^4, 10^5, 10^6$) and check that the runtime grows roughly linearly.
...
(b) Calculate the CUSUM statistic for the Nile volume data. Using the threshold $D = \sqrt{2 \log (n)}$, perform a test to decide if there is a change point. If there is one, where is it? (Where does this threshold come from? Think about how large the CUSUM of pure noise can get -- see the null experiment in Question 1.)
...
(c) Calculate the MOSUM statistic for the Nile volume data using a bandwidth $G = 25$. Using the threshold $D = 3.4$, compute the change point estimators:
    ...
    (The value $D = 3.4$ is not arbitrary: it is approximately the critical value that controls the false-alarm probability at a chosen significance level. The `mosum` package computes such values for you via `mosum:::mosum.criticalValue()`, which you will use indirectly in Question 3.)
...
(b) Setting the argument `method = "AMOC"` (at most one change), use the `cpt.mean()` function on the Nile data set. Do the results agree with your answer from question 2(b)?
...
(c) Use the `cpt.mean()` function to apply the PELT algorithm on the Nile data set. Is the answer the same as part (b)? **Note**: you will need to standardise the data first (subtract the mean and divide by the standard deviation). (Why does this matter? By default `cpt.mean()` looks for changes in mean assuming the data has variance 1; if the series has a different variance the fixed penalty is effectively mis-scaled -- too harsh or too lenient -- and you can get the wrong number of change points.)
...
(f) By investigating the output returned by the `mosum()` function, what are the estimated jump sizes and p-value(s) of the detected change points?
...
The p-value associated to the change is `r nile.cpts.mosum$cpts.info[4]` (this is calculated using the asymptotic null distribution of the test statistic). The jump size is `r nile.cpts.mosum$cpts.info[5]`.
